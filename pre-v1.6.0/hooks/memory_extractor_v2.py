...
def extract_tool_sequences(entries: list[dict], max_results: int = 5) -> list[dict]:
    ...
    sequences = []
    tool_history = []  # (name, input_summary, index)
...
def main():
    try:
        ...
        # --- Partie 1: Metriques (via lib/transcript.py) ---
        metrics = {}
        if transcript_path:
        ...
        # --- Partie 2: Extraction heuristique ---
        entries = []
        if transcript_path:
        ...
        for mem in decisions:
            mem["importance_score"] = importance_defaults.get("decision", 5.5)
            all_memories.append(mem)
