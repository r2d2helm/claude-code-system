...
def _compute_recency_boost(created_at: str) -> float:
    ...
    try:
        # Parse ISO timestamp (tolerant aux formats)
        ts_str = created_at.replace("+", " +").replace("-", " -", 3) if created_at else ""
        # Simplified: just check age in days
        now = datetime.now()
        # Try to parse the timestamp
        for fmt in ("%Y-%m-%dT%H:%M:%S.%f%z", "%Y-%m-%dT%H:%M:%S%z",
                     "%Y-%m-%dT%H:%M:%S.%f", "%Y-%m-%dT%H:%M:%S"):
            try:
                created = datetime.strptime(created_at[:26], fmt[:len(created_at[:26])])
