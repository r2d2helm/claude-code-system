...
def search_memories(
    ...
    try:
        ...
        # Filtrer par keywords si fournis (post-query pour simplifier le SQL)
        if keywords:
            filtered = []
            kw_lower = [k.lower() for k in keywords]
            for mem in results:
                content_lower = mem.get("content", "").lower()
                tags_str = mem.get("tags", "[]").lower()
                if any(kw in content_lower or kw in tags_str for kw in kw_lower):
                    filtered.append(mem)
            return filtered
...
def upsert_fact(key: str, value: str, confidence: float = 0.5, session_id: str = "") -> None:
    ...
    try:
        ...
        if existing:
            # Merge sessions et augmenter confiance
