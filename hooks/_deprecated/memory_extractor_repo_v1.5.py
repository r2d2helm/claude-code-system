...
from lib.memory import read_recent_entries, consolidate_errors, consolidate_corrections
...
def _load_memory_rules() -> tuple[list, list]:
    """Charge les patterns depuis memory_rules.yaml (single source of truth).

    Retourne (insight_patterns, pattern_patterns) compiles.
    Fallback sur des patterns par defaut si le YAML est absent.
    """
    insight_patterns = []
    pattern_patterns = []
    ...
    try:
        if rules_file.exists():
            ...
            for rule in raw.get("insights", []):
                try:
                    insight_patterns.append(re.compile(rule["pattern"], re.IGNORECASE))
                except (re.error, KeyError):
                    continue

            for rule in raw.get("patterns", []):
                try:
                    pattern_patterns.append(re.compile(rule["pattern"], re.IGNORECASE))
                except (re.error, KeyError):
                    continue
    except Exception:
        pass

    # Fallback si rien n'a ete charge
    if not insight_patterns:
        insight_patterns = [
            re.compile(r"(?:the (?:issue|problem|bug|error) was|root cause)(.{20,200})", re.IGNORECASE),
            re.compile(r"(?:fix(?:ed)?|resolved|solved|corrected)\s*(?:by|with|using|:)(.{20,200})", re.IGNORECASE),
            re.compile(r"(?:solution|workaround|fix)\s*(?:is|was|:)(.{20,200})", re.IGNORECASE),
            re.compile(r"(?:found that|discovered that|turns out)(.{20,200})", re.IGNORECASE),
            re.compile(r"(?:the trick is|key insight|important:)(.{20,200})", re.IGNORECASE),
        ]
    if not pattern_patterns:
        pattern_patterns = [
            re.compile(r"(?:always|never|must|should always|should never)(.{20,150})", re.IGNORECASE),
            re.compile(r"(?:convention|standard|pattern|best practice)\s*(?:is|:)(.{20,150})", re.IGNORECASE),
        ]

    return insight_patterns, pattern_patterns
...
def extract_insights(transcript_path: str, max_lines: int = 200) -> dict:
    ...
    """
    insight_patterns, pattern_patterns = _load_memory_rules()
    insights = []
    patterns = []
    seen_hashes = set()
    errors_count = 0
...
def main():
    try:
        ...
        # --- Partie 3: Consolidation memoire ---
        consolidated_count = 0
        try:
            errors = read_recent_entries(MEMORY_DIR / "errors.jsonl", max_entries=200, max_age_days=7)
            if errors:
                error_patterns = consolidate_errors(errors)
                if error_patterns:
                    # Reecrire le fichier consolide
                    consolidated_file = MEMORY_DIR / "error_patterns.jsonl"
                    consolidated_file.write_text("", encoding="utf-8")
                    for ep in error_patterns:
                        append_jsonl(consolidated_file, ep)
                    consolidated_count += len(error_patterns)

            corrections = read_recent_entries(MEMORY_DIR / "corrections.jsonl", max_entries=100, max_age_days=7)
            if corrections:
                learned = consolidate_corrections(corrections)
                if learned:
                    learned_file = MEMORY_DIR / "learned_corrections.jsonl"
                    learned_file.write_text("", encoding="utf-8")
                    for lc in learned:
                        append_jsonl(learned_file, lc)
                    consolidated_count += len(learned)
        except Exception:
            pass
